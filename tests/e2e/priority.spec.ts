...
test("pinned future plan stays compact above stories; clearing the pin picks the nearest plan", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.reload();
  await page.getByRole("button", { name: "大事件", exact: true }).click();
  await expect(page.locator(".event-card").first()).toContainText("最近计划");
